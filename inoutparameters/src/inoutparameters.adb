...
procedure Inoutparameters is
   procedure Display_Money (Account : Float) is
   begin
      Put_Line ("You currently have :" & Float'Image (Account));
   end Display_Money;

   procedure Add_To_Account (
      Account : in out Float;
      Amount : Float) is
   begin
      Account := Account + Amount;
   end Add_To_Account;

   procedure Apply_Financial_Interests (Account : in out Float; Rate : Float) is
      Gain : Float;
   begin
      Gain := Account * Rate;
